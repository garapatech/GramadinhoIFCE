...
import { avatarToGameAppearance } from "./avatarConfig";
...
function applyAtmosphere(state) {
  ...
  const skyColor = daySkyColor.clone().lerp(weatherSkyColor, 0.58).lerp(duskSkyColor, duskMix * 0.7).lerp(nightSkyColor, nightMix * 0.85);
  const fogColor = dayFogColor.clone().lerp(weatherFogColor, 0.5).lerp(duskFogColor, duskMix * 0.65).lerp(nightFogColor, nightMix * 0.9);
  const groundColor = dayGroundColor.clone().lerp(duskGroundColor, duskMix * 0.7).lerp(nightGroundColor, nightMix * 0.8);

  scene.background.copy(skyColor);
  scene.fog.color.copy(fogColor);
  renderer.setClearColor(skyColor, 1);

  const weatherDim = state.weather.rain * 0.34 + state.weather.cloudMix * 0.14;
  ambient.intensity = Math.max(0.22, 0.28 + state.daylight * 1.55 - weatherDim);
  ambient.color.setHex(state.daylight > 0.45 ? (state.weather.rain ? 0xa8bfd6 : 0xdff3ff) : 0x8ea1ca);

  sun.intensity = Math.max(0.04, 0.05 + state.daylight * 2.35 - state.weather.rain * 1.15 - state.weather.cloudMix * 0.48);
  sun.color.setHex(state.daylight > 0.5 ? (state.weather.rain ? 0xe4e8f4 : 0xfff4d6) : state.daylight > 0.1 ? 0xffcab4 : 0x8fa5ff);
  ...
  const nightFactor = 1 - state.daylight;
  for (const lamp of streetLamps) {
    const flicker = Math.sin(clock.elapsedTime * 4 + lamp.phase) * 0.03 * nightFactor;
    const baseIntensity = 0.08 + nightFactor * 0.7;
    lamp.point.intensity = baseIntensity + lamp.manualBoost + flicker;
    lamp.point.distance = 8.5 + nightFactor * 3.5;
    lamp.head.material.emissiveIntensity = 0.18 + nightFactor * 0.92 + lamp.manualBoost * 0.45;
    lamp.head.material.color.setHex(state.daylight > 0.2 ? 0xf2f4d8 : 0xfff1bd);
  }
...
function createCharacter({
  ...
  function buildArm(side) {
    ...
    const sleeve = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 0.86, 12), shirt);
    sleeve.position.y = -0.4;
    sleeve.rotation.z = sign * 0.01;
...
function createSpeechBubble(text) {
  ...
  const r = 24;
  c.fillStyle = "rgba(255, 255, 248, 0.98)";
  c.strokeStyle = "rgba(24, 40, 30, 0.92)";
  c.lineWidth = 5;
  c.beginPath();
  c.moveTo(r, 2);
  c.lineTo(w - r, 2);
...
function updateRemotes(dt, time) {
  for (const r of remotePlayers.values()) {
    if (r.glitchTimer && r.glitchTimer > 0) {
      ...
      r.group.position.y = Math.sin(time * 14 + (r.glitchSeed || 0)) * 0.06;
      continue;
    }
    if (r.celebrateTimer && r.celebrateTimer > 0) {
      r.celebrateTimer -= dt;
      animateCelebrate(r.rig.refs, time + (r.celebrateSeed || 0), 1);
      r.group.position.y = Math.abs(Math.sin((time + (r.celebrateSeed || 0)) * 9)) * 0.08;
      continue;
    }
...
function triggerLocalEmote(kind = "dance", duration = 8.0) {
  ...
  pushEmoteBubble("__local__", kind);
  if (kind === "laugh" || kind === "wave" || kind === "point" || kind === "think") {
...
function createBike(x, z, rotation = 0) {
  ...
  interactables.push({
    ...
    npcDuration: 1.3,
    isDisabledForPlayer: () => playerState.ridingBike === bikeState,
    npcDisabled: () => bikeState.mounted,
    interact() {
...
function createCampusBus(routePoints) {
  ...
  function moveAlongRoute(dt, time) {
    ...
    const next = route[(state.routeIndex + 1) % route.length];
    ...
    if (state.dwellTimer > 0) {
      state.dwellTimer = Math.max(0, state.dwellTimer - dt);
      bus.position.copy(current.position);
      state.facingYaw = Math.atan2(dx, dz);
      bus.rotation.y = lerpAngle(bus.rotation.y, state.facingYaw, 0.18);
    ...
    while (remaining > 0) {
      const distanceLeft = segmentLength - state.segmentProgress;
      const step = Math.min(distanceLeft, remaining);
      state.segmentProgress += step;
      state.wheelSpin += step / 0.34;
      remaining -= step;

      const t = segmentLength <= 0.0001 ? 1 : state.segmentProgress / segmentLength;
      bus.position.set(
        current.position.x + dx * t,
        0,
        current.position.z + dz * t
      );
      state.facingYaw = Math.atan2(dx, dz);
      bus.rotation.y = lerpAngle(bus.rotation.y, state.facingYaw, 0.16);

      if (state.segmentProgress >= segmentLength - 0.0001) {
        state.routeIndex = (state.routeIndex + 1) % route.length;
        state.segmentProgress = 0;
        state.dwellTimer = route[state.routeIndex].dwell;
        bus.position.copy(route[state.routeIndex].position);
        bus.position.y = Math.sin(time * 2.1) * 0.02;
...
function createLamp(x, z) {
  ...
  function toggleLamp() {
    lampState.manualBoost = lampState.manualBoost > 0.1 ? 0 : 0.32;
  }
  interactables.push({
    kind: "lamp",
    label: "Poste",
    ...
    update() {
      ...
      head.material.emissiveIntensity += pulse * (0.5 + lampState.manualBoost);
    }
  });
}
...
const keydownHandler = (event) => {
  ...
  if (event.code === "Digit2") queuedEmoteKind = "think";
...
function updateNpc(npc, dt, time) {
  ...
  if (
    human &&
    npc.chatterCooldown <= 0 &&
    npc.talkCooldown <= 0 &&
    npc.pause <= 0 &&
    !npc.dancing &&
    (npc.state === "idle" || npc.state === "wander") &&
    human.distance <= npc.awarenessRadius * 0.82
  ) {
    const line = pickRandomLine(npc);
    npc.previewLine = line;
    pushBubble({ group: npc.group, key: npc.bubbleKey }, line);
    npc.talkCooldown = 1.8 + rand(0, 1.4);
    npc.chatterCooldown = getNpcChatterDelay(npc);
  }
  ...
  if (
    ...
    if (shouldReact) {
      ...
      if (npc.talkCooldown <= 0 && human.distance <= npc.reactionRadius + 1.5) {
        speak(getNpcReactionLine(npc, human, human.distance), npc.name);
        npc.talkCooldown = 4.5 + rand(0, 3.5);
      }
    }
  }
