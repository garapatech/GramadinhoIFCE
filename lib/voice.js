...
export function createVoiceChat({ onChange, sendReady, sendSignal } = {}) {
  ...
  function snapshot() {
    ...
    const listenerCount = Array.from(knownPlayers.values()).filter(
    ...
    return {
      ...
      status,
      error,
      peerCount: peerList.filter((peer) => peer.hasAudio || peer.status === "connected").length,
      listenerCount,
      peers: peerList,
    };
  }
  ...
  function ensurePeer(remoteId, initiator = false) {
    if (!enabled || !localStream || !remoteId || remoteId === localId) return null;
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (next === "connected") {
        ...
        closePeer(remoteId);
        if (!closed && enabled && player?.voiceEnabled && shouldInitiate(remoteId)) {
          window.setTimeout(() => {
            if (!closed && enabled && knownPlayers.get(remoteId)?.voiceEnabled) {
    ...
    emit();
    if (initiator) void createOffer(peer);
    return peer;
  }

  function connectReadyPeers() {
    if (!enabled || !localStream) return;
    for (const player of knownPlayers.values()) {
      if (!player.voiceEnabled) continue;
      if (shouldInitiate(player.id)) {
  ...
  function addPlayer(player) {
    const normalized = rememberPlayer(player);
    if (normalized?.voiceEnabled && enabled && shouldInitiate(normalized.id)) {
      ensurePeer(normalized.id, true);
    }
    emit();
  }
